...
def process_OPT_keywords():
    all_opt_records = getAllOptRecords(cur)
    for opt_record in all_opt_records:
        if opt_record[0] == 488:
            print(opt_record)
            tab = opt_record[5]
            function = opt_record[6]
            keyword = opt_record[3]
            related_opt_records = getRelatedOptRecords(cur, (tab, function, keyword))
            for related_opt_record in related_opt_records:
                print(related_opt_record)


# def process_OPT_keywords():
#     all_opt_records = getAllOptRecords(cur)
#     for opt_record in all_opt_records:
#         opt_record_id = opt_record[0]
#         keyword_data = list(opt_record[2:10]) # make tuple list so it can be edited
#         keyword_data[2] = 'OPT'
#         tab = keyword_data[3]
...
#         update_opt_records(cur, opt_record_id, tab, function, keyword)
#         con.commit()
...
process_OPT_keywords() # Deal with all OR keywords conditions
